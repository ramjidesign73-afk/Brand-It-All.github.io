...
app.post('/api/submit', async (req, res) => {
  try {
    ...
    if (emailResult.status === 'rejected') {
      console.error('[Email error]', emailResult.reason?.message || emailResult.reason);
      return res.status(500).json({ error: `Email failed: ${emailResult.reason?.message || 'Unknown error'}` });
    }
